    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item
          ...
          >
          <a-tree
            v-if="dataRoleType === '2' || dataRoleType === '3'"
            checkable
            :treeData="groupList"
...
<script>
  import {editDataRoleGroups,queryDepartDataRoleGroup,queryallDepartGroup } from '@/api/api'
  import { getUsercenterAction } from '@/api/manage';

  export default {
    name: "SysDepartDataRoleModal",
    components: {
    ...
    methods: {
      ...
      loadDepartGroups(departId){
        queryDepartDataRoleGroup({departId:departId}).then((res)=>{
          if(res.success){
            if(res.result!=null){
      ...
      loadDepartDataRole(departId){
        ...
        var url = "/sysdepart/sysDepart/queryDepartDataRoleByDepartId?departId="+departId;
      ...
      handleSubmit () {
        ...
        this.form.validateFields((err, values) => {
          if (!err) {
            ...
            let obj;
            obj=editDataRoleGroups(formData);
