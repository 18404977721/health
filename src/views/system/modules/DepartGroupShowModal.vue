<template>
  <a-drawer
    ...
    >
      ...
      <div slot="header" style="color:red;">共{{dataCount}}个单位</div>
    </a-list>
  </a-drawer>

</template>
<script>
  import {queryDepartGroupDepartsName} from '@/api/api'
  ...
  export default {
    ...
    methods: {
      show(groupId){
        ...
        that.visible = true;
        queryDepartGroupDepartsName({groupId:that.groupId}).then((res)=>{
      ...
      save(item){
        ...
        if(sortNo==''){
          ...
          var url = "/sys/departGroup/updateDepartGroupDepartSort?detailId="+id+'&sortNo='+sortNo;
      ...
      del(item){
        var id= item.id;
        let that = this;
        var url = "/sys/departGroup/deleteDepartGroupDepartDetail?detailId="+id;
        putUsercenterAction(url).then((res) => {
          if (res.success) {
