<template>
  ...
  <a-drawer
    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        <a-form-item label="可交办单位分配" :labelCol="labelCol" :wrapperCol="wrapperCol" >
          <a-tree
            checkable
            :treeData="groupList"
...
<script>
  import {editSzfAssignGroups,queryDepartSzfAssignGroup,queryallDepartGroup } from '@/api/api'
  export default {
    name: "SysDepartSzfAssignModal",
    components: {
    ...
    methods: {
      ...
      loadUserGroups(departId){
        queryDepartSzfAssignGroup({departId:departId}).then((res)=>{
          if(res.success){
      ...
      handleSubmit () {
        ...
        this.form.validateFields((err, values) => {
          if (!err) {
            ...
            let obj;
            obj=editSzfAssignGroups(formData);
