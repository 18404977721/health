    ...
    <a-drawer
        ...
        <a-spin :spinning="confirmLoading">
            <a-form :form="form">
                ...
                <a-form-item label="权限分配" :labelCol="labelCol" :wrapperCol="wrapperCol">
                    <a-input v-model="kfqFlag" placeholder="请参照权限配置规则" disabled/>
                    <a-tree
                            checkable
                            :treeData="kfqFlagData"
                            :checkedKeys="kfqFlagKeys"
                            @check="this.onCheck"
                    >
                    </a-tree>
                </a-form-item>

                <a-form-item label="督导组排序" :labelCol="labelCol" :wrapperCol="wrapperCol">
                    <a-input-number placeholder="" v-decorator="[ 'flag1']"/>
                </a-form-item>

                <a-form-item label="任务批示权限" :labelCol="labelCol" :wrapperCol="wrapperCol">
                    <a-radio-group v-decorator="['flag2']">
                        <a-radio value="1">
                            是
                        </a-radio>
                        <a-radio value="0">
                ...
                <a-form-item label="开发区批示权限" :labelCol="labelCol" :wrapperCol="wrapperCol">
                    <a-radio-group v-decorator="['flag3']">
                        <a-radio value="1">
                            是
                        </a-radio>
                        <a-radio value="0">
                ...
                <a-form-item label="App重要交办显示" :labelCol="labelCol" :wrapperCol="wrapperCol">
                    <a-radio-group v-decorator="['showType']">
                        <a-radio value="1">
                            显示领导照片
                        </a-radio>
                        <a-radio value="0">
...
<script>
  ...
  export default {
    ...
    methods: {
      onCheck(kfqFlagKeys) {
        let that = this
        that.kfqFlagKeys = kfqFlagKeys
        let sum = 0
        for (let i = 0; i < kfqFlagKeys.length; i++) {
          sum += Number(kfqFlagKeys[i])
        }
        that.kfqFlag = sum
      },
      //窗口最大化切换s
      ...
      edit(record) {
        ...
        //debugger
        if (record.kfqFlag==null || record.kfqFlag=='') {
          that.kfqFlagKeys = []
        }
        that.kfqFlag = record.kfqFlag
        that.visible = true
        that.model = Object.assign({}, record)
        that.$nextTick(() => {
          that.form.setFieldsValue(pick(this.model, 'username', 'sex', 'realname', 'email', 'phone', 'kfqFlag', 'flag1', 'flag2', 'flag3', 'showType'))
        })
        // 调用查询用户对应的部门信息的方法
        that.checkedDepartKeys = []
        that.loadCheckedDeparts()
        that.CheckkfqFlag(that.kfqFlag)
      },
      CheckkfqFlag(kfqFlag) {
        let that = this
        if (kfqFlag != 0) {
          var start = 1
          var arr = []
          ...
          for (let i = 0; i < arr.length; i++) {
            that.kfqFlagKeys.push(arr[i].toString())
          }
        } else {
          that.kfqFlagKeys = []
        }
      ...
      loadCheckedDeparts() {
        let that = this
        getUsercenterAction(that.url.userWithDepart, { userId: that.userId }).then((res) => {
      ...
      addDepartsToUser(that, formData) {
        ...
        httpUsercenterAction(httpurl, this.userDepartModel, method).then((res) => {
          if (res.success) {
            this.userDepartModel = {}
          } else {
            that.$message.warning(res.message)
