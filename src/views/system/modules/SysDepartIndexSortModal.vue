...
<script>
  import {queryDepartIndexGroupInfo} from '@/api/api'
  import { putUsercenterAction } from '@/api/manage';

  export default {
    name: "SysDepartIndexSortModal",
    components:{
    },
    data(){
      return {
    ...
    methods: {
      show(departId){
        ...
        this.visible = true;
        queryDepartIndexGroupInfo({departId:that.departId}).then((res)=>{
      ...
      save(item){
        ...
        if(sortNo==''){
          ...
          var url = "/sysdepart/sysDepart/updateDepartIndexGroupSort?indexId="+id+'&sortNo='+sortNo;
