...
<script>
  import {queryDepartSzfCooperationGroupInfo} from '@/api/api'
  import { putUsercenterAction } from '@/api/manage';

  export default {
    name: "SysDepartCooperationSortModal",
    components:{
    },
    data(){
      return {
    ...
    methods: {
      show(departId){
        ...
        this.visible = true;
        queryDepartSzfCooperationGroupInfo({departId:that.departId}).then((res)=>{
      ...
      save(item){
        ...
        if(sortNo==''){
          ...
          var url = "/sysdepart/sysDepart/updateDepartSzfCooperationGroupSort?cooperationId="+id+'&sortNo='+sortNo;
