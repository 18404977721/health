...
<script>
  import {queryDepartSzfAssignGroupInfo} from '@/api/api'
  import { putUsercenterAction } from '@/api/manage';

  export default {
    name: "SysDepartAssignSortModal",
    components:{
    },
    data(){
      return {
    ...
    methods: {
      show(departId){
        ...
        this.visible = true;
        queryDepartSzfAssignGroupInfo({departId:that.departId}).then((res)=>{
      ...
      save(item){
        ...
        if(sortNo==''){
          ...
          var url = "/sysdepart/sysDepart/updateDepartSzfAssignGroupSort?assignId="+id+'&sortNo='+sortNo;
