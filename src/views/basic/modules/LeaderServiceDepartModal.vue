...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      handleSubmit(){
        ...
        let url="/common/leader/saveLeadServiceDepart";
  ...
  watch: {
    visible () {
      let that = this;
      if (this.visible) {
        let url="/common/leader/assignDepartTreeList?parentId="+that.leaderGroupCode;
        getUsercenterAction(url).then((res) => {
          ...
          if(treeList!=null && treeList.length >0){
            ...
            output[this.leaderGroupCode]={
              //title:i.parentId,
              title:"本级政府",
              key:this.leaderGroupCode,
              children:[]
            };
          }
        ...
        //回显所有已选择部门
        let reurl="/common/leader/serviceDepartData";
        let params =  {
          leaderId:that.leaderId
        };
        console.log("请求参数：",params);
        postAction(reurl,params).then((res)=>{
